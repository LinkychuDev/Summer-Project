...
public class PlayerSwing2 : MonoBehaviour
{
    ...
    [Header("Parabola")] [SerializeField] private float maxHeight;
    ...
    private void SwingEvent()
    {
        ...
        var gravityForce = -Mathf.Sin(angle * Mathf.Deg2Rad) * Mathf.Abs(PlayerReferenceManager.instance.gravity);
    ...
    private void ReleaseEvent()
    {
        ...
        // curved velocity
        var curvedLaunch = Mathf.Pow(swingVelocity.magnitude, 0.85f) * launchSpeed;
    ...
    {
        var linePoint = Vector3.Lerp(start, end, timeStep);

        var parabola = -4f * minHeight * (timeStep * timeStep - timeStep);
