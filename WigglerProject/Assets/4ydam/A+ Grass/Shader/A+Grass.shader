Shader "4ydam/A+Grass"
{
	Properties
	{
		...
		[Range(0, 1)] _ShadowStrength("Shadow Strength", Float) = 1
	...
	SubShader
	{
		...
		float4 AGrassPow25(float4 v)
		{
			float4 safeV = max(v, 0.0);
			return safeV * safeV * sqrt(safeV);
		}
		...
		Pass
		{
			...
			float3 AdditionalLightsFlatMask171x( float3 WorldPosition, float2 ScreenUV, float4 ShadowMask )
			{
				float3 Color = 0;
					...
					#else
					#define SUM_LIGHTFLAT(Light)\
						Color += Light.color * ( Light.distanceAttenuation * lerp( 1.0, Light.shadowAttenuation, saturate( _ShadowStrength ) ) );
					#endif
			...
			PackedVaryings VertexFunction( Attributes input  )
			{
				...
				float2 panner63 = (((_WindScroll * _AGrassGlobalWind1Multiplier * 0.3) * _TimeParameters.x) * globalWindDirection + temp_output_61_0) * _WindNoiseTexture_ST.xy + _WindNoiseTexture_ST.zw;
				float4 saferPower72 = abs( SAMPLE_TEXTURE2D_LOD( _WindNoiseTexture,  sampler_WindNoiseTexture,  panner63,   0.0 ) );
				float2 panner74 = ((_TimeParameters.x * (_WindJitter * _AGrassGlobalWind1Multiplier * 0.5)) * globalWindDirection + (temp_output_61_0 * 2.0)) * _WindNoiseTexture_ST.xy + _WindNoiseTexture_ST.zw;
				float4 wind1 = ( AGrassPow25(saferPower72) * SAMPLE_TEXTURE2D_LOD( _WindNoiseTexture,  sampler_WindNoiseTexture,  panner74,   0.0 ) );
				

				float2 panner63_2 = (((_WindScroll2 * _AGrassGlobalWind2Multiplier * 0.3) * _TimeParameters.x) * globalWindDirection + temp_output_61_0) * _WindNoiseTexture2_ST.xy + _WindNoiseTexture2_ST.zw;
				float4 saferPower72_2 = abs( SAMPLE_TEXTURE2D_LOD( _WindNoiseTexture2, sampler_WindNoiseTexture2, panner63_2, 0.0 ) );
				float2 panner74_2 = ((_TimeParameters.x * (_WindJitter2 * _AGrassGlobalWind2Multiplier * 0.5)) * globalWindDirection + (temp_output_61_0 * 2.0)) * _WindNoiseTexture2_ST.xy + _WindNoiseTexture2_ST.zw;
				float4 wind2 = ( AGrassPow25(saferPower72_2) * SAMPLE_TEXTURE2D_LOD( _WindNoiseTexture2, sampler_WindNoiseTexture2, panner74_2, 0.0 ) );
						 ...
						 ) : SV_Target
			{
				...
				#else
				ase_lightAtten = ase_mainLight.distanceAttenuation * lerp( 1.0, ase_mainLight.shadowAttenuation, saturate( _ShadowStrength ) );
				#endif
