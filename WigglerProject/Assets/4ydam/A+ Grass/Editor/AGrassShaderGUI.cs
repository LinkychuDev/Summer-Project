    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        MaterialProperty receiveShadowsProp = FindProperty("_ReceiveShadows", properties);
            ...
            GUILayout.BeginVertical(_sectionBoxStyle);
            {
                ...
                if (receiveShadowsProp.floatValue >= 0.5f)
                {
                    EditorGUILayout.Space(2f);
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Space(8f);
                    DrawRoundInfoBadge(14f, "Controls how strongly shadow attenuation affects the grass. 0 removes shadow influence, 1 uses the full shadow result.");
                    GUILayout.Space(6f);

                    MaterialProperty shadowStrengthProp = FindProperty("_ShadowStrength", properties);
                    EditorGUI.BeginChangeCheck();
                    float newShadowStrength = EditorGUILayout.Slider(new GUIContent("Shadow Strength", "Controls how strongly shadow attenuation affects the grass. 0 removes shadow influence, 1 uses the full shadow result."), shadowStrengthProp.floatValue, 0f, 1f);
                    if (EditorGUI.EndChangeCheck())
                    {
                        shadowStrengthProp.floatValue = newShadowStrength;
                    }

                    if (GUILayout.Button("R", GUILayout.Width(20f), GUILayout.Height(EditorGUIUtility.singleLineHeight)))
                    {
                        shadowStrengthProp.floatValue = 1f;
                    }
