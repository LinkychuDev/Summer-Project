    ...
    public override VisualElement CreateInspectorGUI()
    {
        ...
        runtimeCpuBar = new IMGUIContainer(() =>
        {
            ...
            else
            {
                ...
                if (manager != null)
                {
                    float rendererUsage01 = Mathf.Clamp01(manager.CullingCpuUsagePercent / 100f);
                    string rendererLabel = string.Format("Renderer: {0:0.0}%  ({1:0.000} ms/frame)",
                        manager.CullingCpuUsagePercent,
                        manager.CullingCpuUsageMs);
                    Rect rendererBarRect = GUILayoutUtility.GetRect(18f, 18f, "TextField");
                    EditorGUI.ProgressBar(rendererBarRect, rendererUsage01, rendererLabel);

                    float terrainUsage01 = Mathf.Clamp01(manager.TerrainDetailCullingCpuUsagePercent / 100f);
                    string terrainLabel = string.Format("Terrain: {0:0.0}%  ({1:0.000} ms/frame)",
                        manager.TerrainDetailCullingCpuUsagePercent,
                        manager.TerrainDetailCullingCpuUsageMs);
                    Rect terrainBarRect = GUILayoutUtility.GetRect(18f, 18f, "TextField");
                    EditorGUI.ProgressBar(terrainBarRect, terrainUsage01, terrainLabel);
                    ...
                    EditorGUILayout.LabelField(
                        string.Format("Tracked Terrain Patches: {0}   Culled: {1}", manager.TrackedTerrainDetailPatchCount, manager.CulledTerrainDetailPatchCount),
                        EditorStyles.miniLabel);
                }
            }
